...
const QUERY_INSERT_DARK_MODE = (value: string) =>
  'INSERT OR REPLACE INTO CONFIG (id,key,value) VALUES (1,"DARK_MODE","' +
  value +
  '");';
const QUERY_INSERT_CURRENT_LANG = (value: string) =>
  'INSERT OR REPLACE INTO CONFIG (id,key,value) VALUES (2,"CURRENT_LANG","' +
  value +
  '");';

const QUERY_GET_DARK_MODE = 'SELECT * FROM CONFIG WHERE key="DARK_MODE"';

const QUERY_GET_CURRENT_LANG = 'SELECT * FROM CONFIG WHERE key="CURRENT_LANG"';
...
export class ConfigService {
  ...
  async getModeApp(db: SQLiteDBConnection) {
    const results: any = await db.query(QUERY_GET_DARK_MODE);
  ...
  async getCurrentLang(db: SQLiteDBConnection) {
    const results: any = await db.query(QUERY_GET_CURRENT_LANG);
    if (!!results && results.values.length === 1) {
